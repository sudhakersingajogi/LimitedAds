...
//echo $merchantdir; 
	...
	{
		$deal_pic = time()."_".$_FILES['file']['name'];	
		$this->session->set_userdata($this->session->userdata('Business_Id')."_".'deal_pic',$deal_pic);	
		//echo $deal_pic;
	...
	{
		$deal_pic = time()."_".$_FILES['file']['name'];
		$deal_pics = $this->session->userdata($this->session->userdata('Business_Id')."_".'deal_pic').",".$deal_pic;
		$this->session->set_userdata($this->session->userdata('Business_Id')."_".'deal_pic',$deal_pics);	
		//echo $deal_pic;
	}

		
		if(move_uploaded_file($_FILES['file']['tmp_name'],"Resources/dealpicuploads/".$this->session->userdata('Business_Id')."/".$deal_pic))
		{
		// echo "uploaded";	
		}
...
else
{
	...
	$old_uploaded_pics =	$this->Commonmodel->getonefield($table,$cond,'Deal_Pics');
	$old_uploadedpics = explode(",",$old_uploaded_pics);
	//print_r($old_uploadedpics); exit; 
	
	
	
	if(  $this->session->userdata($sessvari) !='')
	{
		$uploaded_images = $offer_pics;	
		$uploaded_images = 1;
	}
	else{
	//	echo "no pic selected to upload<br>";
	unset($postdata['adddealdata']['Deal_pics']);
	//	print_r($postdata['adddealdata']);
	}

	if(array_key_exists('updatePics',$postdata['adddealdata']) )
	{
		$deleted_images = implode(",",$postdata['adddealdata']['updatePics']);
		$deleted_images = 1;
	}
	// if image/s deleted and uploaded image/s
	
	if($uploaded_images =="1" && $deleted_images=="1")
	{
		$diff = array_diff( $old_uploadedpics, $postdata['adddealdata']['updatePics']);		
		$diff = implode(",",$diff);		
		$setdata['Deal_Pics'] = $this->session->userdata($sessvari).",".$diff;
	}
	elseif($deleted_images =="1" && $uploaded_images =="0") //if image/s added
	{
		$diff = array_diff( $old_uploadedpics, $postdata['adddealdata']['updatePics']);		
		$diff = implode(",",$diff);		
		$setdata['Deal_Pics'] = $diff;
		
	}
	elseif($uploaded_images=="1" && $deleted_images =="0") //if image/s not deleted but uploaded images
	{
		$setdata['Deal_Pics'] = $uploaded_images;
	}
	else
	{
		$setdata['Deal_Pics'] = $old_uploaded_pics;
	}

	$cond = array();
	
	$cond['Deal_Id']	 			= $postdata['adddealdata']['dealid'];
	$cond['Store_Id'] 	 			= $postdata['adddealdata']['Store_Id'];
	
	$setdata['Offer_Name']			= $postdata['adddealdata']['offername']; 
	$setdata['Offer_Description']	= $postdata['adddealdata']['offerdescpr']; 
	$setdata['Category']			= $postdata['adddealdata']['editcateg']['catid']; 	
	...
	$setdata['Deal_Starts'] = $date1;


 	$setdata['Deal_Expires'] = $date2;
...
if(	$this->Commonmodel->update($table,$cond,$setdata) )
{
	...
	$del_imgs = $postdata['adddealdata']['updatePics'];
	
	$path = "Resources/dealpicuploads/".$this->session->userdata('Business_Id')."/";
	foreach($del_imgs as $k=>$v)
	{
		$file = $path.$v;
		unlink($file);
	}
...
public function getprinted_Offers_deals()
{
	...
	if($qry!='0')	
	{
		...
		$qry_total = $this->db->query("select deal.Offer_Name, deal.Deal_Id, DATE_FORMAT(deal.Deal_Starts,'%d-%b-%Y') as Dealstarts,DATE_FORMAT(deal.Deal_Expires,'%d-%b-%Y') as Dealends, CASE 'deal.Deal_Expires' WHEN DATE(deal.Deal_Expires)>(CURDATE()-1) THEN 'Expired' WHEN DATE(deal.Deal_Expires)<=(CURDATE()-1) THEN 'Active' END AS Expired_Active, cat.Category as categoryName from $tab as deal join  categories as cat on cat.Category_Id=deal.Category where deal.Store_Id='".$qry."' and deal.Status='Active' order by deal.Deal_Id DESC ");
...
public function getPrinted_deals()
{
	...
	if($qry!='0')	
	{
		...
		$qry_total = $this->db->query("select deal.Offer_Name, deal.Deal_Id, DATE_FORMAT(deal.Deal_Starts,'%d-%b-%Y') as Dealstarts,DATE_FORMAT(deal.Deal_Expires,'%d-%b-%Y') as Dealends, CASE 'deal.Deal_Expires' WHEN DATE(deal.Deal_Expires)>(CURDATE()-1) THEN 'Expired' WHEN DATE(deal.Deal_Expires)<=(CURDATE()-1) THEN 'Active' END AS Expired_Active, cat.Category as categoryName from add_printed_deal as deal join  categories as cat on cat.Category_Id=deal.Category where deal.Store_Id='".$qry."' and deal.Status='Active' order by deal.Deal_Id DESC ");
...
public function get_edit_printed_offer_deal()
{
	...
	$cond = array();
	$table = $postdata['table'];//'add_printed_deal';
...
//viewprinteddeal strats here
	...
	public function viewprinteddeal()
	{
		...
		$stmt= "select deal.Deal_Id as dealid, deal.Offer_Name as Offer_Name, deal.Offer_Description as Offer_Description, DATE_FORMAT(deal.Deal_Starts,'%d-%b-%Y') as Deal_Starts,DATE_FORMAT(deal.Deal_Expires,'%d-%b-%Y') as Deal_Expires, cat.Category as categories, dealsize.SizeName as SizeName  from add_printed_deal as deal join categories as cat on cat.Category_Id=deal.Category join getdealsizes as dealsize on dealsize.SizeId=deal.Add_Size where deal.Deal_Id=$Deal_Id";
...
public function viewoffereddeal()
{
		$postdata= array();
		$postdata = json_decode(file_get_contents('php://input'),TRUE);
		...
		$stmt= "select deal.Deal_Id as dealid, deal.Offer_Name as Offer_Name, deal.Offer_Description as Offer_Description, DATE_FORMAT(deal.Deal_Starts,'%d-%b-%Y') as Deal_Starts,DATE_FORMAT(deal.Deal_Expires,'%d-%b-%Y') as Deal_Expires, cat.Category as categories from add_deal as deal join categories as cat on cat.Category_Id=deal.Category where deal.Deal_Id=$Deal_Id";
...
//view added deal ends here

public function del_deal_printed()
{
	$postdata = array();
	$postdata = json_decode(file_get_contents('php://input'),TRUE);	
	...
	$cond['Deal_Id'] = $postdata['dealid'];
	$setdata = array();
	
	$setdata['Status'] = "Delete";
